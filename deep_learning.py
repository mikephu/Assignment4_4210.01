...
# SPECIFICATION: This program compares the accuracy of different neural networks models which predict if a test sample 
# is an article of clothing (10 classes) with varying numbers of hidden layers, number of neurons per layer, and learning rates. 
...
def build_model(n_hidden, n_neurons_hidden, n_neurons_output, learning_rate):
    ...
    #output layer
    model.add(keras.layers.Dense(n_neurons_output, activation="softmax"))     # output layer with one neuron for each class and the softmax activation function since the classes are exclusive
